...
FName FBSThetaStarPlanner::GetPlannerId() const
{
	return TEXT("ThetaStar");
}

FBSPlanResult FBSThetaStarPlanner::Plan(const FBSPlanRequest& Request) const
{
	...
	ParentMap.Add(StartKey, Request.StartCell);
	...
	while (OpenSet.Num() > 0)
	{
		...
		if (CurrentNode.Cell == Request.GoalCell)
		{
			...
			Result.Cells = FBSGridUtils::SimplifyPath(Grid, ReconstructPath(ParentMap, Request.StartCell, Request.GoalCell));
		...
		const FIntPoint CurrentParent = ParentMap.FindRef(CurrentKey);
		const int64 ParentKey = FBSGridUtils::MakeCellKey(CurrentParent);
		const double ParentScore = GScore.FindRef(ParentKey);
		...
		for (const FIntPoint& Neighbor : Neighbors)
		{
			...
			double TentativeScore = CurrentScore + FBSGridUtils::GetMoveCost(CurrentNode.Cell, Neighbor) + FBSGridUtils::GetClearancePenalty(Grid, Neighbor, Request.ClearanceWeight);
			FIntPoint BestParent = CurrentNode.Cell;

			if (CurrentNode.Cell != CurrentParent && FBSGridUtils::HasLineOfSight(Grid, CurrentParent, Neighbor))
			{
				TentativeScore = ParentScore + FBSGridUtils::GetMoveCost(CurrentParent, Neighbor) + FBSGridUtils::GetClearancePenalty(Grid, Neighbor, Request.ClearanceWeight);
			...
			ParentMap.Add(NeighborKey, BestParent);
